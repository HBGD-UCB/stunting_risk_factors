---
title: "Risk Factors of Stunting"
author: "UCB-SuperLearner Team"
date: "2018-05-25"
output: 
  html_document:
    keep_md: TRUE
    self_contained: true
editor_options: 
  chunk_output_type: console
---



# Pooled Results





  
## Cumulative incidence of stunting from birth to 24 months  
  
### Child characteristics  
  
#### Birth length  
reference level: <=46.8   
![](risk_factor_report_files/figure-html/plot_pooled-1.png)<!-- -->  
  
#### Birthweight  
reference level: <=2600   
![](risk_factor_report_files/figure-html/plot_pooled-2.png)<!-- -->  
  
#### Calendar month of birth  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-3.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-4.png)<!-- -->  
  
#### Child delivered at home  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-5.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/plot_pooled-6.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-7.png)<!-- -->  
  
#### Vaginal delivery  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-8.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: Wealth Q1   
![](risk_factor_report_files/figure-html/plot_pooled-9.png)<!-- -->  
  
#### nchldlt5  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-10.png)<!-- -->  
  
#### nhh  
reference level: 3 or less   
![](risk_factor_report_files/figure-html/plot_pooled-11.png)<!-- -->  
  
#### Number of rooms in household  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-12.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: Food Secure   
![](risk_factor_report_files/figure-html/plot_pooled-13.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: <=266   
![](risk_factor_report_files/figure-html/plot_pooled-14.png)<!-- -->  
  
#### Maternal age  
reference level: <=20   
![](risk_factor_report_files/figure-html/plot_pooled-15.png)<!-- -->  
  
#### Maternal BMI  
reference level: <=18.7   
![](risk_factor_report_files/figure-html/plot_pooled-16.png)<!-- -->  
  
#### Maternal height  
reference level: <=149.6   
![](risk_factor_report_files/figure-html/plot_pooled-17.png)<!-- -->  
  
#### Maternal parity  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-18.png)<!-- -->  
  
#### Maternal weight  
reference level: <=42.4   
![](risk_factor_report_files/figure-html/plot_pooled-19.png)<!-- -->  
  
#### Mother is unmarried  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-20.png)<!-- -->  
  
#### Mother's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/plot_pooled-21.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: <=24   
![](risk_factor_report_files/figure-html/plot_pooled-22.png)<!-- -->  
  
#### Father's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/plot_pooled-23.png)<!-- -->  
  
  
  
## Point prevalence of severe stunting  
  
### Child characteristics  
  
#### Birth length  
reference level: <=46.8   
![](risk_factor_report_files/figure-html/plot_pooled-24.png)<!-- -->  
  
#### Birthweight  
reference level: <=2600   
![](risk_factor_report_files/figure-html/plot_pooled-25.png)<!-- -->  
  
#### Calendar month of birth  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-26.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-27.png)<!-- -->  
  
#### Child delivered at home  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-28.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/plot_pooled-29.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-30.png)<!-- -->  
  
#### Vaginal delivery  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-31.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: Wealth Q1   
![](risk_factor_report_files/figure-html/plot_pooled-32.png)<!-- -->  
  
#### nchldlt5  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-33.png)<!-- -->  
  
#### nhh  
reference level: 3 or less   
![](risk_factor_report_files/figure-html/plot_pooled-34.png)<!-- -->  
  
#### Number of rooms in household  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-35.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: Food Secure   
![](risk_factor_report_files/figure-html/plot_pooled-36.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: <=266   
![](risk_factor_report_files/figure-html/plot_pooled-37.png)<!-- -->  
  
#### Maternal age  
reference level: <=20   
![](risk_factor_report_files/figure-html/plot_pooled-38.png)<!-- -->  
  
#### Maternal BMI  
reference level: <=18.7   
![](risk_factor_report_files/figure-html/plot_pooled-39.png)<!-- -->  
  
#### Maternal height  
reference level: <=149.6   
![](risk_factor_report_files/figure-html/plot_pooled-40.png)<!-- -->  
  
#### Maternal parity  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-41.png)<!-- -->  
  
#### Maternal weight  
reference level: <=42.4   
![](risk_factor_report_files/figure-html/plot_pooled-42.png)<!-- -->  
  
#### Mother is unmarried  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-43.png)<!-- -->  
  
#### Mother's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/plot_pooled-44.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: <=24   
![](risk_factor_report_files/figure-html/plot_pooled-45.png)<!-- -->  
  
#### Father's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/plot_pooled-46.png)<!-- -->  
  
  
  
## Point prevalence of stunting  
  
### Child characteristics  
  
#### Birth length  
reference level: <=46.8   
![](risk_factor_report_files/figure-html/plot_pooled-47.png)<!-- -->  
  
#### Birthweight  
reference level: <=2600   
![](risk_factor_report_files/figure-html/plot_pooled-48.png)<!-- -->  
  
#### Calendar month of birth  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-49.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-50.png)<!-- -->  
  
#### Child delivered at home  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-51.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/plot_pooled-52.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-53.png)<!-- -->  
  
#### Vaginal delivery  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-54.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: Wealth Q1   
![](risk_factor_report_files/figure-html/plot_pooled-55.png)<!-- -->  
  
#### nchldlt5  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-56.png)<!-- -->  
  
#### nhh  
reference level: 3 or less   
![](risk_factor_report_files/figure-html/plot_pooled-57.png)<!-- -->  
  
#### Number of rooms in household  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-58.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: Food Secure   
![](risk_factor_report_files/figure-html/plot_pooled-59.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: <=266   
![](risk_factor_report_files/figure-html/plot_pooled-60.png)<!-- -->  
  
#### Maternal age  
reference level: <=20   
![](risk_factor_report_files/figure-html/plot_pooled-61.png)<!-- -->  
  
#### Maternal BMI  
reference level: <=18.7   
![](risk_factor_report_files/figure-html/plot_pooled-62.png)<!-- -->  
  
#### Maternal height  
reference level: <=149.6   
![](risk_factor_report_files/figure-html/plot_pooled-63.png)<!-- -->  
  
#### Maternal parity  
reference level: 1   
![](risk_factor_report_files/figure-html/plot_pooled-64.png)<!-- -->  
  
#### Maternal weight  
reference level: <=42.4   
![](risk_factor_report_files/figure-html/plot_pooled-65.png)<!-- -->  
  
#### Mother is unmarried  
reference level: 0   
![](risk_factor_report_files/figure-html/plot_pooled-66.png)<!-- -->  
  
#### Mother's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/plot_pooled-67.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: <=24   
![](risk_factor_report_files/figure-html/plot_pooled-68.png)<!-- -->  
  
#### Father height  
reference level: <=168   
![](risk_factor_report_files/figure-html/plot_pooled-69.png)<!-- -->  
  
#### Father's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/plot_pooled-70.png)<!-- -->  
  
  

# Appendix 1 - Relative Risk Forest Plots
  
## Cumulative incidence of stunting from birth to 24 months  
  
### Child characteristics  
  
#### Birth length  
reference level: <=46.8   
![](risk_factor_report_files/figure-html/rr_forest-1.png)<!-- -->  
  
#### Birthweight  
reference level: <=2600   
![](risk_factor_report_files/figure-html/rr_forest-2.png)<!-- -->  
  
#### Calendar month of birth  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-3.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-4.png)<!-- -->  
  
#### Child delivered at home  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-5.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/rr_forest-6.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-7.png)<!-- -->  
  
#### Vaginal delivery  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-8.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: Wealth Q1   
![](risk_factor_report_files/figure-html/rr_forest-9.png)<!-- -->  
  
#### nchldlt5  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-10.png)<!-- -->  
  
#### nhh  
reference level: 3 or less   
![](risk_factor_report_files/figure-html/rr_forest-11.png)<!-- -->  
  
#### Number of rooms in household  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-12.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: Food Secure   
![](risk_factor_report_files/figure-html/rr_forest-13.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: <=266   
![](risk_factor_report_files/figure-html/rr_forest-14.png)<!-- -->  
  
#### Maternal age  
reference level: <=20   
![](risk_factor_report_files/figure-html/rr_forest-15.png)<!-- -->  
  
#### Maternal BMI  
reference level: <=18.7   
![](risk_factor_report_files/figure-html/rr_forest-16.png)<!-- -->  
  
#### Maternal height  
reference level: <=149.6   
![](risk_factor_report_files/figure-html/rr_forest-17.png)<!-- -->  
  
#### Maternal parity  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-18.png)<!-- -->  
  
#### Maternal weight  
reference level: <=42.4   
![](risk_factor_report_files/figure-html/rr_forest-19.png)<!-- -->  
  
#### Mother is unmarried  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-20.png)<!-- -->  
  
#### Mother's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/rr_forest-21.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: <=24   
![](risk_factor_report_files/figure-html/rr_forest-22.png)<!-- -->  
  
#### Father's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/rr_forest-23.png)<!-- -->  
  
  
  
## Point prevalence of severe stunting  
  
### Child characteristics  
  
#### Birth length  
reference level: <=46.8   
![](risk_factor_report_files/figure-html/rr_forest-24.png)<!-- -->  
  
#### Birthweight  
reference level: <=2600   
![](risk_factor_report_files/figure-html/rr_forest-25.png)<!-- -->  
  
#### Calendar month of birth  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-26.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-27.png)<!-- -->  
  
#### Child delivered at home  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-28.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/rr_forest-29.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-30.png)<!-- -->  
  
#### Vaginal delivery  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-31.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: Wealth Q1   
![](risk_factor_report_files/figure-html/rr_forest-32.png)<!-- -->  
  
#### nchldlt5  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-33.png)<!-- -->  
  
#### nhh  
reference level: 3 or less   
![](risk_factor_report_files/figure-html/rr_forest-34.png)<!-- -->  
  
#### Number of rooms in household  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-35.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: Food Secure   
![](risk_factor_report_files/figure-html/rr_forest-36.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: <=266   
![](risk_factor_report_files/figure-html/rr_forest-37.png)<!-- -->  
  
#### Maternal age  
reference level: <=20   
![](risk_factor_report_files/figure-html/rr_forest-38.png)<!-- -->  
  
#### Maternal BMI  
reference level: <=18.7   
![](risk_factor_report_files/figure-html/rr_forest-39.png)<!-- -->  
  
#### Maternal height  
reference level: <=149.6   
![](risk_factor_report_files/figure-html/rr_forest-40.png)<!-- -->  
  
#### Maternal parity  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-41.png)<!-- -->  
  
#### Maternal weight  
reference level: <=42.4   
![](risk_factor_report_files/figure-html/rr_forest-42.png)<!-- -->  
  
#### Mother is unmarried  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-43.png)<!-- -->  
  
#### Mother's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/rr_forest-44.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: <=24   
![](risk_factor_report_files/figure-html/rr_forest-45.png)<!-- -->  
  
#### Father's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/rr_forest-46.png)<!-- -->  
  
  
  
## Point prevalence of stunting  
  
### Child characteristics  
  
#### Birth length  
reference level: <=46.8   
![](risk_factor_report_files/figure-html/rr_forest-47.png)<!-- -->  
  
#### Birthweight  
reference level: <=2600   
![](risk_factor_report_files/figure-html/rr_forest-48.png)<!-- -->  
  
#### Calendar month of birth  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-49.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-50.png)<!-- -->  
  
#### Child delivered at home  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-51.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/rr_forest-52.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-53.png)<!-- -->  
  
#### Vaginal delivery  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-54.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: Wealth Q1   
![](risk_factor_report_files/figure-html/rr_forest-55.png)<!-- -->  
  
#### nchldlt5  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-56.png)<!-- -->  
  
#### nhh  
reference level: 3 or less   
![](risk_factor_report_files/figure-html/rr_forest-57.png)<!-- -->  
  
#### Number of rooms in household  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-58.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: Food Secure   
![](risk_factor_report_files/figure-html/rr_forest-59.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: <=266   
![](risk_factor_report_files/figure-html/rr_forest-60.png)<!-- -->  
  
#### Maternal age  
reference level: <=20   
![](risk_factor_report_files/figure-html/rr_forest-61.png)<!-- -->  
  
#### Maternal BMI  
reference level: <=18.7   
![](risk_factor_report_files/figure-html/rr_forest-62.png)<!-- -->  
  
#### Maternal height  
reference level: <=149.6   
![](risk_factor_report_files/figure-html/rr_forest-63.png)<!-- -->  
  
#### Maternal parity  
reference level: 1   
![](risk_factor_report_files/figure-html/rr_forest-64.png)<!-- -->  
  
#### Maternal weight  
reference level: <=42.4   
![](risk_factor_report_files/figure-html/rr_forest-65.png)<!-- -->  
  
#### Mother is unmarried  
reference level: 0   
![](risk_factor_report_files/figure-html/rr_forest-66.png)<!-- -->  
  
#### Mother's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/rr_forest-67.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: <=24   
![](risk_factor_report_files/figure-html/rr_forest-68.png)<!-- -->  
  
#### Father height  
reference level: <=168   
![](risk_factor_report_files/figure-html/rr_forest-69.png)<!-- -->  
  
#### Father's education  
reference level: Q1   
![](risk_factor_report_files/figure-html/rr_forest-70.png)<!-- -->  
  
  

# Appendix 2 - Population Attributable Risk Forest Plots

  
## Cumulative incidence of stunting from birth to 24 months  
  
### Child characteristics  
  
#### Birth length  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-1.png)<!-- -->  
  
#### Birthweight  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-2.png)<!-- -->  
  
#### Calendar month of birth  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-3.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-4.png)<!-- -->  
  
#### Child delivered at home  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-5.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-6.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-7.png)<!-- -->  
  
#### Vaginal delivery  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-8.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-9.png)<!-- -->  
  
#### nchldlt5  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-10.png)<!-- -->  
  
#### nhh  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-11.png)<!-- -->  
  
#### Number of rooms in household  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-12.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-13.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-14.png)<!-- -->  
  
#### Maternal age  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-15.png)<!-- -->  
  
#### Maternal BMI  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-16.png)<!-- -->  
  
#### Maternal height  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-17.png)<!-- -->  
  
#### Maternal parity  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-18.png)<!-- -->  
  
#### Maternal weight  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-19.png)<!-- -->  
  
#### Mother is unmarried  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-20.png)<!-- -->  
  
#### Mother's education  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-21.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-22.png)<!-- -->  
  
#### Father's education  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-23.png)<!-- -->  
  
  
  
## Point prevalence of severe stunting  
  
### Child characteristics  
  
#### Birth length  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-24.png)<!-- -->  
  
#### Birthweight  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-25.png)<!-- -->  
  
#### Calendar month of birth  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-26.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-27.png)<!-- -->  
  
#### Child delivered at home  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-28.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-29.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-30.png)<!-- -->  
  
#### Vaginal delivery  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-31.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-32.png)<!-- -->  
  
#### nchldlt5  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-33.png)<!-- -->  
  
#### nhh  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-34.png)<!-- -->  
  
#### Number of rooms in household  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-35.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-36.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-37.png)<!-- -->  
  
#### Maternal age  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-38.png)<!-- -->  
  
#### Maternal BMI  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-39.png)<!-- -->  
  
#### Maternal height  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-40.png)<!-- -->  
  
#### Maternal parity  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-41.png)<!-- -->  
  
#### Maternal weight  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-42.png)<!-- -->  
  
#### Mother is unmarried  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-43.png)<!-- -->  
  
#### Mother's education  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-44.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-45.png)<!-- -->  
  
#### Father's education  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-46.png)<!-- -->  
  
  
  
## Point prevalence of stunting  
  
### Child characteristics  
  
#### Birth length  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-47.png)<!-- -->  
  
#### Birthweight  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-48.png)<!-- -->  
  
#### Calendar month of birth  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-49.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-50.png)<!-- -->  
  
#### Child delivered at home  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-51.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-52.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-53.png)<!-- -->  
  
#### Vaginal delivery  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-54.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-55.png)<!-- -->  
  
#### nchldlt5  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-56.png)<!-- -->  
  
#### nhh  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-57.png)<!-- -->  
  
#### Number of rooms in household  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-58.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-59.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-60.png)<!-- -->  
  
#### Maternal age  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-61.png)<!-- -->  
  
#### Maternal BMI  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-62.png)<!-- -->  
  
#### Maternal height  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-63.png)<!-- -->  
  
#### Maternal parity  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-64.png)<!-- -->  
  
#### Maternal weight  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-65.png)<!-- -->  
  
#### Mother is unmarried  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-66.png)<!-- -->  
  
#### Mother's education  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-67.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-68.png)<!-- -->  
  
#### Father height  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-69.png)<!-- -->  
  
#### Father's education  
reference level: NA   
![](risk_factor_report_files/figure-html/par_forest-70.png)<!-- -->  
  
  

# Appendix 3 - Population Attributable Fraction Forest Plots

  
## Cumulative incidence of stunting from birth to 24 months  
  
### Child characteristics  
  
#### Birth length  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-1.png)<!-- -->  
  
#### Birthweight  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-2.png)<!-- -->  
  
#### Calendar month of birth  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-3.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-4.png)<!-- -->  
  
#### Child delivered at home  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-5.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-6.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-7.png)<!-- -->  
  
#### Vaginal delivery  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-8.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-9.png)<!-- -->  
  
#### nchldlt5  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-10.png)<!-- -->  
  
#### nhh  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-11.png)<!-- -->  
  
#### Number of rooms in household  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-12.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-13.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-14.png)<!-- -->  
  
#### Maternal age  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-15.png)<!-- -->  
  
#### Maternal BMI  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-16.png)<!-- -->  
  
#### Maternal height  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-17.png)<!-- -->  
  
#### Maternal parity  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-18.png)<!-- -->  
  
#### Maternal weight  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-19.png)<!-- -->  
  
#### Mother is unmarried  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-20.png)<!-- -->  
  
#### Mother's education  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-21.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-22.png)<!-- -->  
  
#### Father's education  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-23.png)<!-- -->  
  
  
  
## Point prevalence of severe stunting  
  
### Child characteristics  
  
#### Birth length  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-24.png)<!-- -->  
  
#### Birthweight  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-25.png)<!-- -->  
  
#### Calendar month of birth  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-26.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-27.png)<!-- -->  
  
#### Child delivered at home  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-28.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-29.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-30.png)<!-- -->  
  
#### Vaginal delivery  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-31.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-32.png)<!-- -->  
  
#### nchldlt5  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-33.png)<!-- -->  
  
#### nhh  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-34.png)<!-- -->  
  
#### Number of rooms in household  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-35.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-36.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-37.png)<!-- -->  
  
#### Maternal age  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-38.png)<!-- -->  
  
#### Maternal BMI  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-39.png)<!-- -->  
  
#### Maternal height  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-40.png)<!-- -->  
  
#### Maternal parity  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-41.png)<!-- -->  
  
#### Maternal weight  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-42.png)<!-- -->  
  
#### Mother is unmarried  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-43.png)<!-- -->  
  
#### Mother's education  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-44.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-45.png)<!-- -->  
  
#### Father's education  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-46.png)<!-- -->  
  
  
  
## Point prevalence of stunting  
  
### Child characteristics  
  
#### Birth length  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-47.png)<!-- -->  
  
#### Birthweight  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-48.png)<!-- -->  
  
#### Calendar month of birth  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-49.png)<!-- -->  
  
#### Calendar month of measurement  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-50.png)<!-- -->  
  
#### Child delivered at home  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-51.png)<!-- -->  
  
#### Child's sex  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-52.png)<!-- -->  
  
#### Stunted at enrollment  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-53.png)<!-- -->  
  
#### Vaginal delivery  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-54.png)<!-- -->  
  
  
### Household characteristics  
  
#### Asset based wealth index  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-55.png)<!-- -->  
  
#### nchldlt5  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-56.png)<!-- -->  
  
#### nhh  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-57.png)<!-- -->  
  
#### Number of rooms in household  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-58.png)<!-- -->  
  
#### USAID Household Food Insecurity Access Scale (HFIAS) category  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-59.png)<!-- -->  
  
  
### Maternal characteristics  
  
#### Gestational age at birth  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-60.png)<!-- -->  
  
#### Maternal age  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-61.png)<!-- -->  
  
#### Maternal BMI  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-62.png)<!-- -->  
  
#### Maternal height  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-63.png)<!-- -->  
  
#### Maternal parity  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-64.png)<!-- -->  
  
#### Maternal weight  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-65.png)<!-- -->  
  
#### Mother is unmarried  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-66.png)<!-- -->  
  
#### Mother's education  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-67.png)<!-- -->  
  
  
### Paternal characteristics  
  
#### Father age  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-68.png)<!-- -->  
  
#### Father height  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-69.png)<!-- -->  
  
#### Father's education  
reference level: NA   
![](risk_factor_report_files/figure-html/paf_forest-70.png)<!-- -->  
  
  
